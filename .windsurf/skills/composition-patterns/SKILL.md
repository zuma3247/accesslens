---
name: accesslens-composition-patterns
description: React composition patterns for the AccessLens accessibility dashboard. Use when refactoring components with boolean prop proliferation, building flexible component libraries, or designing reusable APIs for the audit results components (Issue Cards, Score Ring, Heatmap, Filters).
license: MIT
metadata:
...
Composition patterns for building flexible, maintainable React components in the AccessLens accessibility dashboard. Avoid boolean prop proliferation by using compound components, lifting state, and composing internals.

## When to Apply

Reference these guidelines when:
- Refactoring Issue Card components with many display variants
- Building reusable UI components for the design system
- Designing flexible component APIs for the audit results panel
- Working with compound components (IssueCard, FilterBar, EmulationWidget)
- Creating provider components for shared state (Theme, Audit Results, Emulation)
...
```tsx
// Collapsed issue card - minimal display
function CollapsedIssueCard({ issue, onExpand }: CollapsedProps) {
  return (
    <IssueCard.Root onClick={onExpand}>
      <IssueCard.Header>
        <SeverityBadge severity={issue.severity} />
        <IssueCard.Title criterion={issue.wcagCriterion} />
        <IssueCard.Count count={issue.affectedCount} />
      </IssueCard.Header>
      <IssueCard.ExpandChevron />
    </IssueCard.Root>
  );
}

// Expanded issue card with full details
function ExpandedIssueCard({ issue, onCollapse }: ExpandedProps) {
  return (
    <IssueCard.Root expanded>
      <IssueCard.Header>
        <SeverityBadge severity={issue.severity} />
        <IssueCard.Title criterion={issue.wcagCriterion} />
      </IssueCard.Header>
      ...
      <IssueCard.Body>
        <IssueCard.Description text={issue.description} />
        <IssueCard.CodeSnippet code={issue.codeSnippet} />
        <IssueCard.FixSuggestion suggestion={issue.fixSuggestion} />
        
        {issue.hasBeforeAfter && (
          <IssueCard.BeforeAfterLink type={issue.beforeAfterType} />
        )}
      </IssueCard.Body>
...
// Compact card for mobile/small screens
function CompactIssueCard({ issue }: CompactProps) {
  return (
    <IssueCard.Root compact>
      <IssueCard.Header>
        <SeverityBadge severity={issue.severity} small />
        <IssueCard.Title criterion={issue.wcagCriterion} truncate />
      </IssueCard.Header>
    </IssueCard.Root>
  );
}
...
Each variant is explicit about what it renders. No hidden conditionals.

---

### ARCHITECTURE-2: Use Compound Components for EmulationWidget
...
Consumers compose exactly what they need. Subcomponents access shared state through context.
...
### STATE-2: Decouple State Management from UI
...
function CopyAllCriticalButton({ issues }: { issues: Issue[] }) {
  ...
  if (criticals.length === 0) {
    return <Badge>No critical violations</Badge>;
  }
...
Each implementation is explicit, self-contained, and documents itself through the function name.

---

### PATTERNS-2: Prefer Composing Children Over Render Props
